...
public class GuideAllotmentServiceImpl implements GuideAllotmentService {
	...
	public String createBatch(CreateBatch createBatch,String createdBy) throws ConflictException,DataIntegrityViolationException{
		...
		if(createBatch.getUgOrPg().equals("UG"))
		{
			...
			try{
				...
				UgGuideAllotmentGuide test = ugGuideRepo.save(new UgGuideAllotmentGuide(batchId, createBatch.getGuide().getUserId(),
					createBatch.getCoguide().getUserId(), createBatch.getSession(),createdBy,simpleDateFormat.format(new Date())));
				if(test.equals(null))
		...
		else if(createBatch.getUgOrPg().equals("PG"))
		{
			...
			try{
				...
				PgGuideAllotmentGuide test = pgGuideRepo.save(new PgGuideAllotmentGuide(batchId, createBatch.getGuide().getUserId(), 
					createBatch.getCoguide().getUserId(), createBatch.getSession(),createdBy,simpleDateFormat.format(new Date())));
				if(test.equals(null))
					throw new ConflictException("Unable to create batch.");
		...
		if(updatedBatch.getUgOrPg().equals("UG"))
		{
			try
			{
				...
				if(updatedBatch.getGuide()!=null)
					guideId = updatedBatch.getGuide().getUserId();
				String coGuideId=null;
				if(updatedBatch.getCoguide()!=null)
					coGuideId = updatedBatch.getCoguide().getUserId();
	...
	BatchData formBatchFromUgGuide(UgGuideAllotmentGuide guide)
	{
		...
		if (guideId != null)
			currGuide = staffRepo.findByUserId(guideId);
		if (coGuideId != null)
			currCoGuide = staffRepo.findByUserId(coGuideId);
		List<String> ids = new ArrayList<String>();
		for(UgGuideAllotmentStudent stud : currStudents)
		{
			ids.add(stud.getStudentId());
		}
		return new BatchData(studRepo.findByUserIdIn(ids),currGuide.orElse(null),currCoGuide.orElse(null),guide.getSession(),"UG",guide.getBatchId());		
	}
